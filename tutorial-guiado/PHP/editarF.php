...
if (isset($erros) && count($erros) > 0) {
    ...
    header("Location: ../View/editar-fluxo.php?idFluxo=". $_POST['idFluxos']);
	 ...
	 try {
        ...
        $stmt = $pdo->prepare('UPDATE fluxos SET nome = :nome, rotina = :rotina, usuario = :usuario, revisaoTexto = :revisaoTexto, revisaoPassos = :revisaoPassos, ambienteDev = :ambienteDev, ambienteProd = :ambienteDev WHERE idFluxos=:id');
        ...
        $retorno  = $stmt->execute();
         ...
         $stmt = $pdo->prepare("INSERT INTO revisao (nome, rotina, dataRevisao, usuario, texto, passos , revisaoTexto, revisaoPassos, ambienteDev, ambienteProd) VALUES(:nome, :rotina, :dataRevisao, :usuario, :texto, :passos, :revisaoTexto, :revisaoPassos, :ambienteDev, :ambienteProd)");
	     $stmt->execute(array(':nome' => $nome, ':rotina' => $rotina, ':dataRevisao' => $data, ':usuario' => $usuario, ':texto' => $texto, ':passos' => $passos, ':revisaoTexto' => $revisaoTexto, ':revisaoPassos' => $revisaoPassos, ':ambienteDev' => $ambienteDev, ':ambienteProd' => $ambienteProd));
  
  if (! $retorno){
      $erros[] = utf8_encode('Não foi possivel atualizar o fluxo, Tente Novamente');
  }else {
       ?> <script>alert("Fluxo editado com sucesso!");</script> <?php
